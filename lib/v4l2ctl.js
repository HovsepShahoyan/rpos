...
(function (v4l2ctl) {
    ...
    v4l2ctl.Controls = {
        UserControls: {
            ...
            contrast: new UserControl(0, { range: { min: 0, max: 100 } }),
            ...
            digital_zoom: new UserControl(1, { range: { min: 1, max: 4 } }),
            ...
            pallete: new UserControl(1, { lookupSet: [{ value: 0, desc: 'Disabled' }, { value: 1, desc: '50 Hz' }, { value: 2, desc: '60 Hz' }, { value: 3, desc: 'Auto' }] }),
                 
            //palette: new UserControl(0, { range: { min: 0, max: 14 } }),
            //ip_address: new UserControl("192.168.0.86", { stringify: (val) => val }),
            //username: new UserControl("admin", { stringify: (val) => val }),
            //password: new UserControl("admin", { stringify: (val) => val }),
        },
        CodecControls: {
    ...
    //     console.log("CALEED22")
    //     console.log("CALLING execV4l2 with command:", cmd);
    ...
    function execV4l2(cmd) {
        ...
        try {
            // Check if the command includes brightness
            if (cmd.includes('brightness')) {
                // Extract the brightness value from the command (e.g., brightness=65)
                const brightness = cmd.split('=')[1];
                console.log(`[V4L2] Brightness value extracted: ${brightness}`);
    
                // Call the cmdClient.setBrightness function to set the brightness on the camera
                cmdClient.setBrightness(brightness);
            }

            // Check if the command includes zoom
            if (cmd.includes('digital_zoom')) {
                // Extract the zoom level from the command (e.g., LD=1, LD=2, etc.)
                const zoomLevel = cmd.split('=')[1];
                console.log(`[V4L2] Zoom level extracted: ${zoomLevel}`);

                // Call the cmdClient.setZoom function to set the zoom on the camera
