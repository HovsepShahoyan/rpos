...
const doubleClick = require('./double_click');
...
var PTZDriver = (function () {
    function PTZDriver(config) {
        ...
        this.process_ptz_command = function (command, data) {
            console.log("the command is :", command, data)
            if (command === 'gotohome') {
                CommandClient.setMiddle(true);
                ...
                if (_this.pelcod) {
                    ...
                    if (p < 0 && t > 0)
                        ...
                        let a1 = Math.abs(parseFloat(p));
                        CommandClient.calculateAndSetSpeed(a1);
                      //  console.log("During Right we have the value of a1: ", a1)
                        CommandClient.setRight(true); }
                    else if (p < 0){
                        _this.pelcod.left(true);
                        let a2 = Math.abs(parseFloat(p));
                        CommandClient.calculateAndSetSpeed(a2);
                      //  console.log("During Left we have the value of a2: ", a2)
                        CommandClient.setLeft(true); }
                    else if (t > 0) {
                        _this.pelcod.up(true);
                        let a3 = Math.abs(parseFloat(t));
                        CommandClient.calculateAndSetSpeed(a3);
                     //   console.log("During up we have the value of a3: ", a3)
                        CommandClient.setUp(true); }
                    else if (t < 0) {
                        let a4 = Math.abs(parseFloat(t));
                        CommandClient.calculateAndSetSpeed(a4);
                       // console.log("During Down we have the value of a4: ", a4)
                        CommandClient.setDown(true); }
                    else if (t == 0 && p == 0 && z == 0) {
                        CommandClient.setPTZStop(true);
                    }
                    ...
                    if (z > 0) {
                        if (global.currentDayZoom < 6 && global.currentDayZoom >= 1) {
                            global.currentDayZoom = global.currentDayZoom + 1;
                            if (global.currentDayZoom >= 4) {
                            global.currentIRZoom = global.currentIRZoom + 1;
                            console.log("Current IR zoom: ", global.currentIRZoom);
                            CommandClient.setZoomPTZ(global.currentIRZoom);
                        }
                        }
                        CommandClient.setDayZoomForPTZ(global.currentDayZoom);

                        _this.pelcod.zoomIn(true); }
                    if (z < 0) {
                        if (global.currentDayZoom <= 6 && global.currentDayZoom > 1) {
                            global.currentDayZoom = global.currentDayZoom - 1;
                            if (global.currentDayZoom >= 3) {
                                global.currentIRZoom = global.currentIRZoom - 1;
                                console.log("Current IR zoom: ", global.currentIRZoom);
                                CommandClient.setZoomPTZ(global.currentIRZoom);
                            }
                ...
                if (_this.visca) {
                    ...
                    console.log("pan_speed ", visca_pan_speed)
                ...
                var t = 0.0;
                var z = 0.0;
                let xcoord = commandClient.convertX(data.pan);
                let ycoord = commandClient.convertY(data.tilt);

                console.log("---------: p and t converted", xcoord, ycoord, global.currentDayZoom)

                doubleClick.onDoubleClick(xcoord, ycoord, global.currentDayZoom);
                try {
                    p = parseFloat(data.pan);
                }
                catch (err) { }
                ...
                console.log("Set Constrast " + data.value);
                if (_this.rposAscii)
                    _this.stream.write(command + '\t' + data.value + '\n');
                v4l2ctl_1.v4l2ctl.SetContrast(data.value);
            }
            else if (command === 'contrast') {
                console.log("Set Constrast " + data.value);
                if (_this.rposAscii)
           ...
           //     console.log("Focus " + data.value);
                ...
                if (_this.pelcod) {
                    if (data.value < 0) {
                        ...
                        flag = 1;
                        CommandClient.setNearFocus(true);
                        console.log("PTZ focus < 0 ")
                    }
                    else if (data.value > 0) {
                        _this.pelcod.focusFar(true);
                        flag = 1;
                        CommandClient.setFarFocus(true);
                        console.log("PTZ focus > 0 ")
                    }
                    else {
                ...
                CommandClient.setStopFocus(true);
                if(flag === 0) {
                    CommandClient.pollRangeFinder();
                    //console.log("Range finder")
                }
                flag = 0;
                if (_this.rposAscii)
                    _this.stream.write(command + '\n');
...
//PTZDriver.setZooming = setZooming;
