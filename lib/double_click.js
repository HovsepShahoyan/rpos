...
function onDoubleClick(x, y, zoom) {
 ...
 if (presetsFlag) {
    ...
    const panelX = 1920 - panelW - 10;  // Right side of screen
    const panelY = 80;                  // From top (since invY = 1080 - y)
    
    // Check if click is inside the entire presets panel
    if (x >= panelX && x <= panelX + panelW && invY >= panelY && invY <= panelY + panelH) {
        // First check for numpad clicks
        ...
        const slotsStartY = delTop + btnH + 30;  // Start of slot list (matches Python)
        const slotH = 30;                    // Height of each slot
        const slotW = panelW - 40;           // Width of slot text area

        // ... Add/Delete button handling remains the same ...

        // 3) Marker slots (1-10) - PRECISELY aligned with Python drawing
        for (let i = 0; i < 10; i++) {
            // Python draws text at: y = slots_start_y + i * (slot_h + 8)
            // The text baseline is at this y position
            // For OpenCV putText, the y coordinate is the baseline of the text
            // So the clickable area should be approximately from y-25 to y+5
            const textBaselineY = slotsStartY + i * (slotH + 8);
            const slotTop = textBaselineY - 25;  // Text typically extends ~25px above baseline
            const slotBottom = textBaselineY + 5; // And ~5px below
            
            // X coordinates match exactly with text drawing
            if (x >= panelX + 20 && x <= panelX + 20 + slotW &&  // Left/right bounds
                invY >= slotTop && invY <= slotBottom) {         // Top/bottom bounds
                ...
                if (lastPresetClick === i) {
                    try {
                        ...
                        if (pos && pos.az != null && pos.el != null && 
                            parseInt(pos.zoomDay) != null && parseInt(pos.zoomIR) != null) {
        ...
        // Click was inside panel but not on any specific element
        return;
    }
}
