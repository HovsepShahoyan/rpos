...
const PIP_TOGGLE_PATH       = "/tmp/pip_toggle.json";

// ─────────────────────────────────────────────────────────────────────────────
// 1) “Default” contents for each JSON—used when creating them if they don’t exist
// ─────────────────────────────────────────────────────────────────────────────

const defaultPipToggle = {
  pip_visible: 0, 
};

const defaultMenuOverlay = {
  Flag: 0,
  nc_xy_mode: 0,
  nc_x_value: "0",
  nc_y_value: "0"
...
// ─────────────────────────────────────────────────────────────────────────────
// 2) In‐memory state for menu/numpad/preset/PIP
// ─────────────────────────────────────────────────────────────────────────────
...
let ncXYMode = false; // false = angle, true = XY
let ncXValue = "0";
let ncYValue = "0";
let pipVisible = false; // PIP visibility state
...
ensureJsonFile(PIP_TOGGLE_PATH, defaultPipToggle);

try {
  const raw = fs.readFileSync(MENU_OVERLAY_PATH, "utf-8");
  const overlay = JSON.parse(raw);
  ncXYMode = !!overlay.nc_xy_mode;
  ncXValue = overlay.nc_x_value || "0";
  ncYValue = overlay.nc_y_value || "0";
} catch (e) {
  console.warn("Could not load menu_overlay.json XY state, using defaults.");
}
...
function updateMenuOverlayJSON() {
  const menuOverlay = {
    Flag: menuFlag ? 1 : 0,
    nc_xy_mode: ncXYMode ? 1 : 0,
    nc_x_value: ncXValue,
    nc_y_value: ncYValue
  };
  try {
    ...
    console.error("Error updating menu_overlay.json:", error);
  }
}

function handleNumpadButton(buttonText) {
  if (activeField === "nc_x" || activeField === "nc_y") {
    handleNCNumpadButton(buttonText);
    return;
  }
  switch (buttonText) {
    case "OK":
...
function handleNCNumpadButton(buttonText) {
  switch (buttonText) {
    case "OK":
      activeField = null;
      break;
    case "C":
      if (activeField === "nc_x") ncXValue = "0";
      else if (activeField === "nc_y") ncYValue = "0";
      break;
    default:
      if (activeField === "nc_x") ncXValue = (ncXValue === "0") ? buttonText : ncXValue + buttonText;
      else if (activeField === "nc_y") ncYValue = (ncYValue === "0") ? buttonText : ncYValue + buttonText;
      break;
  }
  updateMenuOverlayJSON();
}
...
function onDoubleClick(x, y, zoom) {
  ...
  if (x >= 1270 && x <= 1350 && y >= 20 && y <= 60) {
    // 1) Toggle PIP visibility
    pipVisible = !pipVisible;
    const pipToggle = { pip_visible: pipVisible ? 1 : 0 };
    try {
      fs.writeFileSync(PIP_TOGGLE_PATH, JSON.stringify(pipToggle, null, 2));
    } catch (error) {
      console.error("Error toggling pip_toggle.json:", error);
    }
    return;
  }
  ...
  if (presetsFlag) {
      ...
      const panelW = 520, panelH = 550; // match overlay panel size
      ...
      if (x >= panelX + 10 && x <= panelX + panelW - 10 &&
        ...
        try {
          ...
          const values = cmdClient.getDestinationSK42ForPresets(cmdClient.X, cmdClient.Y, cmdClient.azimuthDeg, cmdClient.distance, cmdClient.Mc)
          
          positions[currentPresetIndex] = { az: Az, el: El, zoomIR: irZoom, zoomDay: dayZoom, X: values.X, Y: values.Y };
          fs.writeFileSync(
      ...
      for (let i = 0; i < 10; i++) {
        ...
        // Adjust slot width to cover right-aligned coordinates
        if (
          x    >= panelX  &&
          x    <= panelX + panelW - 30 &&
          invY >= top    -10  &&
  ...
  if (menuFlag) {
    ...
    if (scaledX >= 150 && scaledX <= 270 && invY >= 220 && invY <= 270) {
    ...
    if (scaledX >= 20 && scaledX <= 400 && invY >= 20 && invY <= 80) {
      ...
      if (ncXYMode) {
        const cameraX = cmdClient.X;
        const cameraY = cmdClient.Y;
        const targetX = parseFloat(ncXValue);
        const targetY = parseFloat(ncYValue);
        cmdClient.northConnectByXY(cameraX, cameraY, targetX, targetY);
        console.log("NorthConnect XY mode:", cameraX, cameraY, targetX, targetY);
      }
      else {
        // Angle mode 
    ...
    if (scaledX >= 30 && scaledX <= 390 && invY >= 100 && invY <= 140) {
      if (ncXYMode) {
        activeField = "nc_x";
        // Optionally update overlay_data/menu_overlay.json here
      } else {
        activeField = "field1";
        updateMenuInputJSON();
      }
