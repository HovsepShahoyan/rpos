...
document.addEventListener('DOMContentLoaded', function() {
  try {
    // Ensure video-wrapper exists and is positioned
    var vw = document.querySelector('.video-wrapper');
    if (!vw) {
      console.error('DEBUG: .video-wrapper not found');
      // attempt to find closest container
      vw = document.getElementById('fastlive-page') || document.body;
    } else {
      // make sure it can contain absolutely positioned children
      var cs = window.getComputedStyle(vw);
      if (cs.position === 'static') {
        vw.style.position = 'relative';
        console.log('DEBUG: set .video-wrapper style.position = relative');
      } else {
        console.log('DEBUG: .video-wrapper already positioned:', cs.position);
      }
      // give a visible border so we can see the region
      vw.style.outline = '2px dashed red';
    }

    // Remove existing overlay container if present (clean start)
    var existing = document.getElementById('overlay-container');
    ...
    console.log('DEBUG: overlay-container added to .video-wrapper');

    // Create a visible test crosshair using inline SVG data URL
    var img = document.createElement('img');
    img.id = 'debug-cross';
    // Small SVG crosshair (red)
    ...
    img.addEventListener('load', function() {
      console.log('DEBUG: inline crosshair image loaded successfully');
    });
    img.addEventListener('error', function(e) {
      console.error('DEBUG: inline crosshair failed to load', e);
    });

    // Now try to fetch the coords file for stream 1 (adjust path if you use proxy)
    var coordsPath = '/overlay/overlay_coords1.json'; // change if your path differs
    console.log('DEBUG: attempting fetch of coords at', coordsPath);
    var xhr = new XMLHttpRequest();
    xhr.open('GET', coordsPath + '?_=' + Date.now(), true);
    xhr.onreadystatechange = function() {
      if (xhr.readyState !== 4) return;
      console.log('DEBUG: coords fetch status', xhr.status);
      if (xhr.status >= 200 && xhr.status < 300) {
        try {
          var data = JSON.parse(xhr.responseText);
          console.log('DEBUG: coords JSON:', data);
        } catch (e) {
        ...
        console.warn('DEBUG: Check your overlay server or proxy path and CORS if different origin');
      }
    };
    ...
    // Also log iframe sizing info to detect zero-height issues
    var iframe = document.getElementById('webrtcFrame');
    if (iframe) {
      setTimeout(function() {
