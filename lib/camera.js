...
<div id="logs-page" class="page-content">
  ...
  <div class="card-body">
  <textarea id="logsOutput" class="form-control" rows="15" readonly>Loading logs...</textarea>

  <div style="margin-top:10px;">
    <a class="standard-button" onclick="loadLogs()">Refresh</a>

    <!-- Simple anchor link to the protected download endpoint (works with session cookie) -->
    <a id="downloadLogsLink" class="standard-button" href="/api/downloadLoginAttempts" download>
      Download
    </a>
  </div>
</div>
        ...
        if (xhr.readyState === 4) {
            ...
            if (xhr.status === 200) {
                try {
                    ...
                    output.value = "⚠️ Failed to parse JSON: " + e;
                }
            } else {
                output.value = "⚠️ Failed to load logs (status " + xhr.status + ")";
            }
        }
    };
    xhr.send();
