        ...
        <div class="video-container">
                ...
                <script>
                    ...
                    function calculateNewPositionEncoderCrosshair(clickX, clickY, screenWidth, screenHeight, currentHorizontal, currentVertical, zoom) {
                        ...
                        var correctionCoefficientH = DAY_ZOOM_COEFFICIENT_GRID[zoom] ? DAY_ZOOM_COEFFICIENT_GRID[zoom][0] : 1.0;
                        var correctionCoefficientV = DAY_ZOOM_COEFFICIENT_GRID[zoom] ? DAY_ZOOM_COEFFICIENT_GRID[zoom][1] : 1.0;
                    ...
                    function doPTZMove(clickX, clickY, zoom) {
                        // Get current PTZ positions (simplified - in real implementation, get from cmdClient)
                        var currentHorizontal = 135500; // Placeholder
                        var currentVertical = 131072;   // Placeholder
                        ...
                        // Send PTZ command
                        console.log('PTZ Move: Click at (' + clickX + ', ' + clickY + '), New position: (' + newHorizontal + ', ' + newVertical + ')');

                        // Send PTZ command using the existing PTZ position controls
                        var formData = 'UserControls.enc_deg_movement_az=' + newHorizontal + '&UserControls.enc_deg_movement_el=' + newVertical;
                        fetch('/', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/x-www-form-urlencoded'
                    ...
                    function handleVideoClick(event) {
                        ...
                        doPTZMove(percentX, percentY, currentZoom);
