...
<!-- Fast Live Page -->
<div id="fastlive-page" class="page-content">
    <div class="page-header">
    ...
    <div class="stream-interface">
        ...
        <div class="controls-panel">
            ...
            <div class="control-section">
                ...
                <div class="image-controls">
                    <div class="adjustment-row">
                        ...
                        <input type="range" min="0" max="100" value="50" class="adjustment-slider" id="brightnessSlider">
                        <span id="brightnessValue">50</span>
                    </div>
                    <div class="adjustment-row">
                        <span>Contrast</span>
                        <input type="range" min="0" max="100" value="50" class="adjustment-slider" id="contrastSlider">
            ...
            <div class="control-section">
                ...
                <input type="range" class="zoom-slider" min="1" max="30" value="1" id="zoomSlider">
            </div>
            ...
            <div class="control-section">
                ...
                <div class="ptz-controls-fast">
                    <div class="ptz-grid-fast">
                        ...
                        <button class="ptz-arrow-fast" onclick="movePTZ('up')">↑</button>
                        <button class="ptz-arrow-fast" onclick="movePTZ('up-right')">↗</button>
                        <button class="ptz-arrow-fast" onclick="movePTZ('left')">←</button>
                        <button class="ptz-center-fast" onclick="movePTZ('center')">⏺</button>
                        <button class="ptz-arrow-fast" onclick="movePTZ('right')">→</button>
                        <button class="ptz-arrow-fast" onclick="movePTZ('down-left')">↙</button>
                        <button class="ptz-arrow-fast" onclick="movePTZ('down')">↓</button>
            ...
            <div class="control-section">
                ...
                <div class="focus-controls">
                    <button class="focus-btn" onclick="adjustFocus('near')">Near -</button>
                    <button class="focus-btn" onclick="adjustFocus('far')">Far +</button>
                </div>
            </div>
        </div>
    </div>
...
function initControls() {
    ...
    if (brightnessSlider) {
        brightnessSlider.addEventListener('input', function() {
            ...
            // document.getElementById('liveStream').style.filter = 'brightness(' + (value/50) + ')';
        });
    }
    ...
    if (contrastSlider) {
        contrastSlider.addEventListener('input', function() {
            ...
            // Apply contrast to video stream
            // document.getElementById('liveStream').style.filter = 'contrast(' + (value/50) + ')';
        });
    }
    ...
    if (zoomSlider) {
        zoomSlider.addEventListener('input', function() {
            const zoomValue = document.getElementById('zoomValue');
            if (zoomValue) zoomValue.textContent = this.value + 'x';
...
document.addEventListener('DOMContentLoaded', function() {
  try {
    // Configuration: show multiple crosshairs at once?
    var SHOW_MULTIPLE_CROSSHAIRS = true; // <- set true so both crosses are visible

    // Find video-wrapper elements. If more than one, treat them as separate streams.
    ...
    // Utility: create overlay container inside a wrapper (id includes index)
    function createOverlayForWrapper(wrapperEl, idx) {
      ...
      // ensure wrapper is positioned to contain absolute children
      ...
      // visible outline for debugging
      wrapperEl.style.outline = '2px dashed red';
    ...
    // Map of containers by stream number (1-based)
    var containersByStream = {};

    // If multiple wrappers, assume wrappers[0] -> stream1, wrappers[1] -> stream2, etc.
    // If single wrapper but multiple streams, we create multiple crosshairs inside that one container.
    if (wrappers.length >= 2) {
      // create one container per wrapper. stream numbers start at 1
      wrappers.forEach(function(w, i) {
        var streamNum = i + 1;
        containersByStream[streamNum] = createOverlayForWrapper(w, streamNum);
      });
    } else {
      // single wrapper: create one overlay container and use it for all streams
      containersByStream[1] = createOverlayForWrapper(wrappers[0], 1);
      // point stream2 also to same container (we use IDs to keep elements separate)
      containersByStream[2] = containersByStream[1];
    }

    // Create (or ensure) a crosshair element inside a given container for streamNumber
    function createCrosshairInContainer(streamNumber, container) {
      ...
      if (!img) {
        ...
        // Basic style (same as your original)
        img.style.position = 'absolute';
        img.style.left = '50%';
        img.style.top = '50%';
        img.style.transform = 'translate(-50%, -50%)';
        img.style.pointerEvents = 'none';
        img.style.zIndex = String(10000 + streamNumber); // deterministic stacking
        img.style.width = 'auto';
        img.style.height = 'auto';
        img.style.maxWidth = '100px';
        img.style.maxHeight = '100px';

        // Debugging listeners and fallback
        img.addEventListener('load', function() {
          console.log('DEBUG: crosshair loaded for stream', streamNumber, 'in container', container.id,
                      'src:', img.src, 'natural:', img.naturalWidth, 'x', img.naturalHeight);
        });
        img.addEventListener('error', function(e) {
          console.error('DEBUG: crosshair failed to load for stream', streamNumber, 'in container', container.id, e);
          console.error('DEBUG: attempted path:', img.src);
          setTimeout(function() {
            if (img.naturalWidth === 0) {
              ...
              img.src = 'data:image/svg+xml;utf8,' +
                encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48">' +
                                  ...
                                  '<line x1="0" y1="24" x2="48" y2="24" stroke="red" stroke-width="1"/>' +
                                  '<line x1="24" y1="0" x2="24" y2="48" stroke="red" stroke-width="1"/>' +
                                  '</svg>');
            }
          }, 700);
        });
      ...
      // point to image on same origin for reliability
    ...
    function updateCrosshairPosition(streamNumber, coords) {
      ...
      if (coords.x !== undefined && coords.y !== undefined) {
        if (coords.x <= 1 && coords.y <= 1) {
          ...
          // optionally remove translate for pixel coords:
          // el.style.transform = 'translate(0,0)';
        }
        console.log('DEBUG: updated crosshair position for stream', streamNumber, coords.x, coords.y);
      }
    }

    // If you want single-active behavior (still supported), switching hides others
    var currentStream = 1;
    window.switchCrosshair = function(streamNumber) {
      currentStream = streamNumber;
      if (!SHOW_MULTIPLE_CROSSHAIRS) {
        // hide all crosshairs, then show requested
        var all = document.querySelectorAll('.crosshair-overlay');
        all.forEach(function(el) {
          el.style.display = (el.dataset.stream === String(streamNumber)) ? 'block' : 'none';
        });
      } else {
        // ensure requested one is visible
        var el = document.getElementById('crosshair-stream-' + streamNumber);
        if (el) el.style.display = 'block';
      }
      console.log('DEBUG: switched to crosshair for stream', streamNumber);
    };

    // Test loader that shows where each image is placed and whether it loads
    window.testCrosshair = function() {
      console.log('Testing crosshair loading for streams 1 and 2...');
      [1, 2].forEach(function(s) {
        var src = window.location.origin + '/tmp/active_cross' + s + '.png?_=' + Date.now();
        var t = new Image();
        t.onload = function() { console.log('✓ test load OK for stream', s, src); };
        t.onerror = function() { console.error('✗ test load FAILED for stream', s, src); };
        t.src = src;
      });

      // Also print containers
      console.log('DEBUG containersByStream:', containersByStream);
      Object.keys(containersByStream).forEach(function(k) {
        var c = containersByStream[k];
        console.log('DEBUG: stream', k, 'container id:', c && c.id, 'parent:', c && c.parentNode && c.parentNode.tagName);
      });
    };

    // Auto-run test after short delay
    setTimeout(window.testCrosshair, 800);

    // Optional: coordinate fetcher (unchanged)
    function fetchAndUpdateCoords(streamNumber) {
      var coordsPath = '/overlay/overlay_coords' + streamNumber + '.json';
      console.log('DEBUG: attempting fetch of coords at', coordsPath);

      var xhr = new XMLHttpRequest();
      xhr.open('GET', coordsPath + '?_=' + Date.now(), true);
      xhr.onreadystatechange = function() {
        if (xhr.readyState !== 4) return;
        console.log('DEBUG: coords fetch status for stream', streamNumber, xhr.status);
        if (xhr.status >= 200 && xhr.status < 300) {
          try {
            var data = JSON.parse(xhr.responseText);
            updateCrosshairPosition(streamNumber, data);
          } catch (e) {
            console.error('DEBUG: coords JSON parse error', e, xhr.responseText);
          }
        } else {
          console.warn('DEBUG: coords file not available or server returned', xhr.status, xhr.statusText);
        }
      };
      xhr.onerror = function(e) {
        console.error('DEBUG: coords fetch XHR error', e);
      };
      xhr.send();
    }

    console.log('DEBUG: overlay init complete. SHOW_MULTIPLE_CROSSHAIRS =', SHOW_MULTIPLE_CROSSHAIRS);

    // iframe size log (unchanged)
    var iframe = document.getElementById('webrtcFrame');
    if (iframe) {
      setTimeout(function() {
        var r = iframe.getBoundingClientRect();
        console.log('DEBUG: iframe rect', r.width, 'x', r.height, 'top', r.top, 'left', r.left);
      }, 300);
    } else {
      console.warn('DEBUG: webrtcFrame not found');
    }
