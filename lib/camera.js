...
var Camera = (function () {
    function Camera(config, webserver) {
        ...
        webserver.post('/api/scheduleReboot', function (req, res) {
            // Debug logging (helpful while debugging)
            ...
            // Auth check - keep your admin check
            if (!req.session || !req.session.user || req.session.user.role !== 'admin') {
                console.warn('Unauthorized scheduleReboot attempt from', req.ip, 'sessionUser=', req.session && req.session.user);
                return res.status(403).send('Forbidden');
            }

            // Accept both JSON and urlencoded: look for time in req.body or, as fallback, parse raw body.
            ...
            // fallback: if time still missing, try to parse raw body (urlencoded or JSON)
            if (!time && req.rawBody) {
                // try JSON parse
                try {
            ...
            // parse the time input (browser datetime-local) into a Date
            var rebootDate = new Date(time);
            if (isNaN(rebootDate.getTime())) {
                // attempt to accept "YYYY-MM-DDTHH:MM" without seconds
            ...
            // compute delay in minutes from now (round up)
            var now = new Date();
            var diffMs = rebootDate.getTime() - now.getTime();
            var diffMinutes = Math.ceil(diffMs / 60000);
            if (diffMinutes <= 0) diffMinutes = 1; // schedule at least 1 minute ahead

            console.log('Scheduling reboot in minutes:', diffMinutes, 'target date:', rebootDate.toString());

            // Build the sudo shutdown command, using +minutes format
            // If you prefer using /sbin/shutdown path, change 'shutdown' to '/sbin/shutdown'
            var child = spawn('sudo', ['-S', 'shutdown', '-r', '+' + diffMinutes], { stdio: ['pipe', 'pipe', 'pipe'] });
            ...
            // If a password was provided, write it (not recommended). If empty, sudo must be NOPASSWD for shutdown.
            if (password && password.length > 0) {
                try {
                    child.stdin.write(password + '\n');
                } catch (e) {
                    console.warn('Failed writing sudo password to stdin:', e && e.message);
                }
            }
            try { child.stdin.end(); } catch (e) {}

            child.on('error', function (err) {
                console.error('spawn error scheduling reboot:', err);
                return res.status(500).json({ error: 'Failed to start scheduler', details: err.message });
            });

            child.on('close', function (code) {
                console.log('shutdown process exited code=', code, 'stderr=', stderr);
                if (code === 0) {
                    return res.json({ ok: true, message: 'Reboot scheduled in ' + diffMinutes + ' minute(s) (' + rebootDate.toString() + ')' });
                } else {
                    return res.status(500).json({ error: 'Scheduler failed', code: code, stderr: stderr });
                }
            });
        });
        var _this = this;
    ...
    <script>
        ...
        document.querySelectorAll('.nav-link').forEach(link => {
            link.addEventListener('click', (e) => {
                ...
                if (tgt !== 'logs') {
                    // Hide everything related to logs to avoid "sticking" visibility
                    ...
                    // Show wrapper and trigger load
                    logsWrapper.style.display = '';
...
function switchStream(streamNumber) {
    console.log("Switching to stream " + streamNumber);
    // Toggle active class on buttons
    document.querySelectorAll('.stream-btn').forEach(function(btn) {
        btn.classList.toggle('active', btn.textContent.includes(streamNumber.toString()));
    });
    // Here you would switch the video source
    // Example: document.getElementById('liveStream').src = 'stream' + streamNumber + '.m3u8';
}
