...
var Camera = (function () {
    ...
    webserver.get('/api/currentZoom', (req, res) => {
        const zoom = global.currentDayZoom || 1;
        console.log('Returning current zoom:', zoom);
        res.json({ zoom });
    });
    ...
    webserver.post('/api/setZoomLevel', (req, res) => {
        ...
        console.log('Received setZoomLevel:', level, typeof level);
        if (typeof level === 'number' && level >= 1 && level <= 6) {
            global.currentDayZoom = level;
            console.log('Set global.currentDayZoom to', level);
            res.json({ success: true });
        } else {
    ...
    if (prop) {
        ...
        if (g === 'UserControls' && p === 'day_zoom') {
            ...
            if (index !== -1) {
                ...
                console.log('Updated global.currentDayZoom to', global.currentDayZoom, 'for day_zoom value', value);
            }
        }
    } else {
...
var DAY_ZOOM_COEFFICIENT_GRID = {
                    ...
                    function calculateNewPositionEncoderCrosshair(clickX, clickY, screenWidth, screenHeight, currentHorizontal, currentVertical, zoom, xCrossPos, yCrossPos) {
  ...
  var [correctionCoefficientH, correctionCoefficientV] = DAY_ZOOM_COEFFICIENT_GRID[zoom] || [1.0, 1.0];
  ...
  var relY = 0.5 - (clickY / screenHeight);
                        ...
                        console.log('Calc: zoom=' + zoom + ', fovH=' + fovHorizontal + ', fovV=' + fovVertical + ', coeffH=' + correctionCoefficientH + ', coeffV=' + correctionCoefficientV + ', angleH=' + angleOffsetH + ', angleV=' + angleOffsetV);
                    ...
                    function doPTZMove(clickX, clickY, zoom, currentHorizontal, currentVertical, xCrossPos, yCrossPos) {
                        var result = calculateNewPositionEncoderCrosshair(
                            clickX, clickY, VIDEO_WIDTH, VIDEO_HEIGHT, currentHorizontal, currentVertical, zoom, xCrossPos, yCrossPos
                        );

                        var newHorizontal = result[0];
                        var newVertical = result[1];

                        console.log('PTZ Move: Current pos (' + currentHorizontal + ', ' + currentVertical + '), New pos (' + newHorizontal + ', ' + newVertical + ')');
                    ...
                    async function handleVideoClick(event) {
                        ...
                        console.log('Fetched zoomValue:', currentZoomValue, 'mapped to level:', zoomLevel);
                        ...
                        try {
                            ...
                            console.log('Fetched cross positions:', xCrossPos, yCrossPos);
                        } catch (error) {
                        ...
                        try {
                            ...
                            doPTZMove(percentX, percentY, zoomLevel, currentHorizontal, currentVertical, xCrossPos, yCrossPos);
                        } catch (error) {
        ...
        <div class="controls-panel">
            ...
            <div class="control-section">
                ...
                <div class="angle-info">
                    ...
                    <div style="display: flex; align-items: center; gap: 0.5rem;">D1: <input type="number" min="0" max="59" value="1" class="form-control" style="width: 80px;" onchange="sendControlValue('UserControls', 'alphaD1', this.value)" /></div>
                    <div style="display: flex; align-items: center; gap: 0.5rem;">D2: <input type="number" min="0" max="99" value="1" class="form-control" style="width: 80px;" onchange="sendControlValue('UserControls', 'alphaD2', this.value)" /></div>
                </div>
            </div>
    ...
    <!-- Measure Range -->
    <div class="control-section">
        <div class="control-label">MEASURE RANGE</div>
    ...
    <script>
        ...
        function showToast(message, type = 'info') {
            ...
            // Remove existing toasts to prevent overlapping
...
function setSpeed(speed) {
    ...
    document.querySelectorAll('.speed-btn').forEach(function(btn) {
        ...
        function setDayZoom(direction) {
            ...
            global.currentDayZoom = level; // Update client global
