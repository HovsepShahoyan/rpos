...
var Camera = (function () {
    function Camera(config, webserver) {
        ...
        webserver.get('/api/downloadLoginAttempts', function (req, res) {
            // Require admin role
            ...
            const filePath = '/tmp/login_attempts.json';
            if (fs.existsSync(filePath)) {
                // Use res.download so the browser will treat it as a file attachment
                res.download(filePath, 'login_attempts.json', function (err) {
                if (err) {
                    console.error('Download failed:', err);
                    // If headers were not sent, return 500
...
<div id="logs-page" class="page-content">
  ...
  <div class="logs-controls" style="display:flex; gap:0.6rem; align-items:center; flex-wrap:wrap;">
    ...
    <div style="margin-left:auto; display:flex; gap:0.5rem;">
      ...
      <a id="downloadLogsLink" class="standard-button download-button" href="/api/downloadLoginAttempts" download style="display:none;">Download</a>
    </div>
  </div>

  <div id="logsTableWrapper" style="margin-top:0.8rem;">
    <!-- Table will be injected here -->
    <div id="logsEmpty" style="padding:1rem; color:var(--text-secondary); display:none;">No logs to show.</div>
    <div style="overflow:auto; max-height:420px;">
      <table id="logsTable" class="logs-table" style="width:100%; border-collapse:collapse; display:none;">
        <thead>
          <tr>
            ...
            <th data-col="raw" style="width:1px;"></th>
          </tr>
        </thead>
        <tbody></tbody>
      </table>
