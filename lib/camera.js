...
document.addEventListener('DOMContentLoaded', function() {
  try {
    // Configuration: Only show the active stream's crosshair
    var SHOW_MULTIPLE_CROSSHAIRS = false; // Set to false so only active stream shows
    var currentActiveStream = 1; // Track which stream is currently active

    // Find video-wrapper elements
    ...
    // Utility: create overlay container inside a wrapper
    ...
    // Map of containers by stream number
    var containersByStream = {};
    
    // Single wrapper scenario - create one container for both streams
    containersByStream[1] = createOverlayForWrapper(wrappers[0], 1);
    containersByStream[2] = containersByStream[1]; // Both use same container

    // Store crosshair positions for each stream separately
    var streamPositions = {
      1: { x: 0.5, y: 0.5 }, // Default center position for stream 1
      2: { x: 0.5, y: 0.5 }  // Default center position for stream 2
    };

    // Create crosshair element for a stream
    function createCrosshairInContainer(streamNumber, container) {
      ...
      if (!img) {
        ...
        // Set initial position from stored position
        var pos = streamPositions[streamNumber];
        ...
        img.addEventListener('error', function(e) {
          console.error('DEBUG: crosshair failed to load for stream', streamNumber, e);
          // Fallback to inline SVG
          setTimeout(function() {
            if (img.naturalWidth === 0) {
              console.log('DEBUG: falling back to inline SVG crosshair for stream', streamNumber);
              var color = streamNumber === 1 ? 'red' : 'blue'; // Different colors for debugging
      ...
      // Set initial visibility - only show if this is the active stream
      img.style.display = (streamNumber === currentActiveStream) ? 'block' : 'none';

      return img;
    }

    // Create crosshairs for both streams
    ...
    function updateCrosshairPosition(streamNumber, coords) {
      ...
      // Store the position for this stream
      if (coords.x !== undefined && coords.y !== undefined) {
        if (coords.x <= 1 && coords.y <= 1) {
          streamPositions[streamNumber] = { x: coords.x, y: coords.y };
        } else {
          // Convert pixel coordinates to percentage
          var container = containersByStream[streamNumber];
          if (container) {
            var rect = container.getBoundingClientRect();
            streamPositions[streamNumber] = {
      ...
      // Only update visual position if this is the active stream
      if (streamNumber === currentActiveStream) {
        var container = containersByStream[streamNumber];
        if (!container) return;
        ...
        console.log('DEBUG: updated crosshair position for stream', streamNumber, pos.x, pos.y);
      }
    }

    // Enhanced stream switching function
    window.switchStream = function(streamNumber) {
      console.log('DEBUG: Switching from stream', currentActiveStream, 'to stream', streamNumber);
      ...
      // Show only the crosshair for the active stream
      var activeCrosshair = document.getElementById('crosshair-stream-' + streamNumber);
      if (activeCrosshair) {
        activeCrosshair.style.display = 'block';
        
        // Apply the stored position for this stream
        var pos = streamPositions[streamNumber];
        activeCrosshair.style.left = (pos.x * 100) + '%';
        activeCrosshair.style.top = (pos.y * 100) + '%';
        
        console.log('DEBUG: Activated crosshair for stream', streamNumber, 'at position', pos);
      }
      
      // Update button states
      var buttons = document.querySelectorAll('.stream-btn');
      buttons.forEach(function(btn, idx) {
        if (idx + 1 === streamNumber) {
      ...
      // Optionally fetch new coordinates for the active stream
    ...
    function fetchAndUpdateCoords(streamNumber) {
      ...
      console.log('DEBUG: fetching coords for stream', streamNumber, 'from', coordsPath);
      ...
      xhr.onreadystatechange = function() {
        ...
        if (xhr.status >= 200 && xhr.status < 300) {
          try {
            ...
            console.error('DEBUG: coords JSON parse error', e);
          }
        } else {
          console.warn('DEBUG: coords not available for stream', streamNumber);
        }
      };
      xhr.send();
    }
    ...
    window.testCrosshair = function() {
      ...
      console.log('Current active stream:', currentActiveStream);
      console.log('Stream positions:', streamPositions);
      ...
      [1, 2].forEach(function(s) {
        ...
        var t = new Image();
        t.onload = function() { 
          console.log('✓ Image loads OK for stream', s, src); 
        };
        t.onerror = function() { 
          console.error('✗ Image load FAILED for stream', s, src); 
        };
        t.src = src;
      });
      
      // Show current crosshair states
      var allCrosshairs = document.querySelectorAll('.crosshair-overlay');
      allCrosshairs.forEach(function(el) {
    ...
    // Initialize with stream 1 active
    setTimeout(function() {
      window.switchStream(1);
      window.testCrosshair();
    }, 500);

    // Optionally poll for coordinate updates
    if (false) { // Set to true to enable polling
      setInterval(function() {
        fetchAndUpdateCoords(currentActiveStream);
      }, 2000); // Update every 2 seconds
    }

    console.log('DEBUG: Overlay system initialized. Active stream:', currentActiveStream);
