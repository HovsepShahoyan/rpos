...
var Camera = (function () {
    ...
    webserver.get('/api/currentZoom', (req, res) => {
        ...
        console.log('Returning current zoom:', zoom, 'for stream:', global.currentStream);
        res.json({ zoom });
    });
    ...
    webserver.post('/api/setCurrentStream', (req, res) => {
        ...
        if (typeof stream === 'number' && (stream === 1 || stream === 2)) {
            ...
            console.log('Set current stream to', stream);
            res.json({ success: true });
        } else {
    ...
    if (prop) {
        ...
        if (g === 'UserControls' && p === 'digital_zoom') {
            ...
            if (index !== -1) {
                ...
                console.log('Updated global.currentIRZoom to', global.currentIRZoom, 'for digital_zoom value', value);
            }
        }
    } else {
...
(function(){
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Notify server of current stream
