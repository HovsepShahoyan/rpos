...
class CommandClient {
    ...
    getSTMDData() {
        if (
            ...
            const { curr_pos_Az, curr_pos_El, Encoder_Az, Encoder_El, Enc_Az_crct, Enc_El_crct } = this.latestClientData1.properties;
            return { curr_pos_Az, curr_pos_El, Encoder_Az, Encoder_El, Enc_Az_crct, Enc_El_crct };
        }
        else {
            return {}; 
        }
    ...
    setWithEncDegMovementAz(offsetEncoderUnits) {
        if (
            ...
            const currentEncoderAz = parseInt(this.latestClientData1.properties.Enc_Az_crct, 10);
    
            const degreesPerStep = 360 / Math.pow(2, 18); // ≈ 0.00137329 degrees per unit
            const offsetDegrees = offsetEncoderUnits * degreesPerStep;
    
            const adjustedEncoderAz = currentEncoderAz + offsetEncoderUnits;
            ...
            console.log('[AZ OFFSET] Current Encoder_Az:', currentEncoderAz);
    ...
    setWithDegEncMovementAz(offsetDegrees) {
        if (
            ...
            console.log('[AZ DEGREES] Current Encoder_Az:', currentEncoderAz);
    ...
    setWithEncDegMovementEl(offsetEncoderUnits) {
        if (
            ...
            const currentEncoderEl = parseInt(this.latestClientData1.properties.Enc_El_crct, 10);
    
            const degreesPerStep = 360 / Math.pow(2, 18); // ≈ 0.00137329 degrees per unit
            const offsetDegrees = offsetEncoderUnits * degreesPerStep;
    
            const adjustedEncoderEl = currentEncoderEl + offsetEncoderUnits;
    
            console.log('[AZ OFFSET] Offset in encoder units:', offsetEncoderUnits);
            console.log('[AZ OFFSET] Offset in degrees:', offsetDegrees.toFixed(4));
            console.log('[AZ OFFSET] Current Encoder_Az:', currentEncoderEl);
    ...
    setWithDegEncMovementEl(offsetDegrees) {
        if (
            this.latestClientData1 &&
            this.latestClientData1.properties/* &&
            this.latestClientData1.properties.Enc_El_crct*/
            ...
            console.log('[AZ DEGREES] Offset in degrees:', offsetDegrees);
            console.log('[AZ DEGREES] Converted offset in encoder units:', offsetEncoderUnits);
            console.log('[AZ DEGREES] Current Encoder_El:', currentEncoderEl);
            console.log('[AZ DEGREES] Adjusted Encoder_El:', adjustedEncoderEl);
