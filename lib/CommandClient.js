...
class CommandClient {
    constructor() {
        ...
        this.client.on('data', (data) => {
            ...
            if (this.latestInclinationData && this.latestOdData && this.latestClientData1) {
                ...
                // 5) Azimuth zero hard-coded to 0
                const encoderAzZero = 0;
            ...
            if (this.X  && this.Y && this.azimuthDeg && this.distance && this.Mc ) {
                this.distanceToTarget = this.getDestinationSK42(this.X, this.Y, this.azimuthDeg, this.distance, this.Mc);
                console.log(">>>>>Distance to target is :", this.distanceToTarget)
            }
    ...
    getDestinationSK42(X, Y, azimuthDeg, distance, Mc) {
        ...
        const deltaX = adjustedDistance * Math.cos(this.toRadians(azimuthDeg));
        const deltaY = adjustedDistance * Math.sin(this.toRadians(azimuthDeg));
    ...
    // getGPSData() {
    //     const p = this.latestClientData1 && this.latestClientData1.properties;
    //     if (!p) return {};
      
    //     // 1) Rebuild the raw NMEA fragments
    //     const rawTime = `${p.GPS_0 || ""}${p.GPS_1 || ""}`;     // e.g. "123519"
    //     const rawLat  = `${p.GPS_2 || ""}.${p.GPS_3 || ""}`;     // e.g. "4021.1292"
    //     const rawLon  = `${p.GPS_4 || ""}.${p.GPS_5 || ""}`;     // e.g. "04447.4112"
    //     const rawAlt  = p.GPS_6 || "0";
      
    //     // 2) Convert time string "hhmmss" or "hhmmss.sss" → "hh:mm:ss[.sss]"
    //     function convertGPSTime(ts) {
    //       if (!ts) return null;
    //       // split off fractional seconds if present
    //       const [hms, frac] = ts.split('.');
    //       if (hms.length < 6) return ts;  // weird format, just return raw
    ...
    //     // 3) NMEA ddmm.mmmm or dddmm.mmmm → decimal degrees
    //     function nmeaToDecimal(val) {
    //       const f = parseFloat(val);
    //       if (isNaN(f)) return null;
    //       const deg  = Math.floor(f / 100);
    //       const mins = f - deg * 100;
    //       return deg + mins / 60;
    //     }
      
    //     // 4) Decide if this is raw NMEA (len>3) or plain degrees
    //     function convertGPSLatLon(raw, isLatitude) {
    //       const intLen = raw.split('.')[0].length;
    //       if (intLen > (isLatitude ? 4 : 5)) {
    //         // ddmm.mmmm  or  dddmm.mmmm
    //         return nmeaToDecimal(raw);
    //       } else {
    ...
    //     // 5) Compute converted values
    //     const time      = convertGPSTime(rawTime);
    //     const latitude  = convertGPSLatLon(rawLat, true);
    //     const longitude = convertGPSLatLon(rawLon, false);
    //     const altitude  = parseFloat(rawAlt) || 0;
      
    //     console.debug(
    //       `[GPS DEC] time=${time}, latitude=${latitude}, longitude=${longitude}, altitude=${altitude}`
    //     );
      
    //     // 6) Return exactly the keys your downstream converters expect:
    //     return {
    //       time,       // string like "12:35:19" or null
    ...
    getGPSData() {
        const p = this.latestSnsData && this.latestSnsData.properties;
        if (!p) return {};
      ...
      getXYcoordinates() {
        try {
          ...
          if (isNaN(lat) || isNaN(lon)) {
            console.warn(`[GPS] Bad data, skipping conversion: lat='${latitude}', lon='${longitude}'`);
            return null;
          }
      
          // B) Run the SK42→Cartesian conversion
          const wgs  = new GeoCoordinate(lat, lon, parseFloat(altitude));
          const { x, y } = convertWgsToSk42(wgs);
          console.debug(`[Conversion] x=${x}, y=${y}`);
      
          if (isNaN(x) || isNaN(y)) {
            console.warn(`[Conversion] Got NaN, skipping write: x=${x}, y=${y}`);
            return null;
          }
      
          // C) Write the coords object properly
          const coords = { x, y };
          fs.writeFileSync('/tmp/overlay_coords.json', JSON.stringify(coords));
          console.log(`✅ Wrote /tmp/overlay_coords.json:`, coords);
