...
class CommandClient {
    constructor(cameraConfig = null) {
        if (CommandClient.instance) return CommandClient.instance;
            CommandClient.instance = this;

        // Use provided camera config or fallback to defaults
        this.HOST = cameraConfig?.ip_address || '192.168.0.21';
        this.PORT = cameraConfig?.port || 2222;
        this.HOST1 = cameraConfig?.ip_address_secondary || cameraConfig?.ip_address || '192.168.0.21';
        this.PORT1 = cameraConfig?.port_secondary || 8888;
